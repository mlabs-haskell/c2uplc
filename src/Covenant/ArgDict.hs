...
import Covenant.Unsafe (Arg (UnsafeMkArg), BoundTyVar (BoundTyVar), Id (UnsafeMkId))
import Data.Map (Map)
...
simplePrettyASG lookupNode thisId@(UnsafeMkId i) = \case
  ...
  AValNode _ valInfo -> case valInfo of
    ...
    Cata _ handlers arg -> matchLike "cata" <$> goRef arg <*> traverse goRef handlers
  ...
  where
    ...
    goRef = \case
      ...
      AnArg (UnsafeMkArg argDb' argIx' _) -> do
