...
module Covenant.Transform.Cata (mkCatamorphism) where
  ...
  where
    ...
    genCataPLC (CompN cataFnCount (ArgsAndResult origCataFnArgs _)) nameBase _enc schema = do
        ...
        let lamArgVars = pVar <$> lamArgNames
            {- As with `match`, this is a Haskell-level function which takes the body of a series of nested lambdas as an argument
        ...
        case schema of
            SOPSchema _ -> do
                ...
                fix <- resolveStub "fix"
                pure $ fix # lamBuilder (pCase scrutinee wrappedHandlers)
...
builtinCataForm ::
    ...
    DataEncoding ->
    m (Maybe TyFixerFnData)
builtinCataForm nm _ = case nm of
    "List" -> do
        let cataListTy =
                Comp2 $
                    listT a
